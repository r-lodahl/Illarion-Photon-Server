...
namespace Illarion.Server.Navigation
{
    public sealed class NavigationTest
    {
        ...
        public NavigationTest(ITestOutputHelper output)
        {
            _output = output;
        }
        ...
        public void LocationControlTest(Vector3 oldPosition, Vector3 newPosition, float deltaTime,
            Vector3 expectedResult)
        {
            IServiceProvider serviceProvider = new ServiceCollection().BuildServiceProvider();

            var investigator = new LocationInvestigator(serviceProvider);
